...
use lechange_core::types::{
    ChangeType, ChangedFile, FileOrigin, InputConfig, ProcessedResult,
};
use std::borrow::Cow;
use std::fs;
...
"#;
    ...
    let tests = result
        ...
        .find(|g| g.key == "tests")
        .unwrap();
    assert_eq!(tests.matched_indices.len(), 1);

    // Verify changed_keys and modified_keys
    assert!(outputs.changed_keys.contains(&"frontend".to_string()));
    assert!(outputs.changed_keys.contains(&"backend".to_string()));
    assert!(outputs.changed_keys.contains(&"tests".to_string()));

    // frontend and tests have Modified files, backend has Added
    assert!(outputs.modified_keys.contains(&"frontend".to_string()));
    assert!(outputs.modified_keys.contains(&"tests".to_string()));
    assert!(!outputs.modified_keys.contains(&"backend".to_string()));
}
...
async fn test_soft_fail_skip_same_sha() {
    ...
    assert!(result.diagnostics.iter().any(|d| {
        d.category == lechange_core::types::DiagnosticCategory::SkippedSameSha
    }));
    assert!(result.all_files.is_empty());
}

// 6C variant: Test that fail_on_initial_diff_error=true returns Err
#[tokio::test]
async fn test_fail_on_diff_error() {
    let (_dir, repo_path) = create_test_repo();
    ...
    // This should fail because the initial commit has no parent
    let config = InputConfig {
    ...
    // With only one commit, HEAD^ doesn't exist, so this should error
