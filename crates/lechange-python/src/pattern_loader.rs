...
pub fn load_yaml_patterns(py: Python<'_>, yaml: &str, negation_first: bool) -> PyResult<Vec<Py<PyAny>>> {
    let groups = PatternLoader::load_yaml_groups(yaml, negation_first).map_err(|e| {
        PyErr::new::<crate::error::YamlError, _>(format!("{}", e))
    })?;
