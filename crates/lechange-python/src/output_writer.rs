...
impl PyOutputWriter {
    ...
    fn write_text(
        ...
        OutputWriter::write_text(Path::new(output_dir), name, &refs, separator).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyOSError, _>(format!("{}", e))
        })
    }
    ...
    fn write_json(output_dir: &str, name: &str, values: Vec<String>) -> PyResult<()> {
        let refs: Vec<&str> = values.iter().map(|s| s.as_str()).collect();
        OutputWriter::write_json(Path::new(output_dir), name, &refs).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyOSError, _>(format!("{}", e))
        })
    }
