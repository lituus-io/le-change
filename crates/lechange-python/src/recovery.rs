...
impl PyFileRecovery {
    ...
    fn recover_file(&self, sha: &str, file_path: &str, output_dir: &str) -> PyResult<String> {
        ...
        let result = self.inner.recover_file(sha, file_path, output).map_err(|e| {
            PyErr::new::<crate::error::RecoveryError, _>(format!("{}", e))
        })?;
    ...
    fn recover_files(
        ...
        for fp in &file_paths {
            let result = self.inner.recover_file(sha, fp, output).map_err(|e| {
                PyErr::new::<crate::error::RecoveryError, _>(format!("{}", e))
            })?;
